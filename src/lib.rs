...
impl Storage for InfluxDbStorage {
    ...
    async fn put(&mut self, key: OwnedKeyExpr, sample: Sample) -> ZResult<StorageInsertionResult> {
        ...
        let measurement = match &self.config.strip_prefix {
            Some(prefix) => match key.as_str().strip_prefix(prefix.as_str()) {
        ...
        if let Some(del_time) = self.get_deletion_timestamp(measurement).await? {
            ...
            if sample_ts < del_time {
                debug!(
                    "Received a Sample for {} with timestamp older than its deletion; ignore it",
                    key
                );
        ...
        let (base64, strvalue) = match String::from_utf8(sample.payload.contiguous().into_owned()) {
            Ok(s) => (false, s),
        ...
        let query = InfluxWQuery::new(InfluxTimestamp::Nanoseconds(influx_time), measurement)
            .add_tag("kind", "PUT")
        ...
        debug!("Put {} with Influx query: {:?}", key, query);
    ...
    async fn delete(
        ...
        debug!("Delete {} with Influx query: {:?}", key, query);
        if let Err(e) = self.client.query(&query).await {
            bail!(
        ...
        if let Err(e) = self.client.query(&query).await {
            bail!("Failed to mark measurement {} as deleted : {}", key, e)
        }
    ...
    async fn get(&mut self, key: OwnedKeyExpr, parameters: &str) -> ZResult<Vec<Sample>> {
        ...
        let regex = if let Some(prefix) = &self.config.strip_prefix {
            ...
            let vec = key.strip_prefix(prefix);
            if vec.is_empty() {
                bail!("Received get on key '{}', but the configured strip_prefix='{:?}' is not a prefix of this key", key, self.config.strip_prefix);
            }
            debug!(
                "Get on {} with strip_prefix={} => sub-keyexprs = {:?}",
                key, prefix, vec
            );
            // convert the sub-key expressions into an Influx regex
            key_exprs_to_influx_regex(vec.as_slice())
        } else {
            // convert the key expression into an Influx regex
            key_exprs_to_influx_regex(&[&KeyExpr::from(&key)])
        };
                        ...
                        Ok(retn) => {
                            ...
                            for serie in retn.series {
                                ...
                                for zpoint in serie.values {
                                    ...
                                    result.push(
                                        Sample::new(ke.clone(), value).with_timestamp(timestamp),
                                    );
                                }
                            }
                        }
