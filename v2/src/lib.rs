...
impl InfluxDbStorage {
    fn get_db_name(&self) -> ZResult<String> {
...
impl Storage for InfluxDbStorage {
    ...
    async fn put(
        ...
        let measurement = key.unwrap_or((*NONE_KEY_REF).clone());
        ...
        match self.client.write(&db, stream::iter(zenoh_point)).await {
    ...
    async fn get(
        ...
        let owned_key = key.unwrap_or((*NONE_KEY_REF).clone());

        let db: String = self.get_db_name()?;
        ...
        // Filter Measurement
        if owned_key.to_string().contains('*') {
